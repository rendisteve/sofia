...
class P2h extends CI_Controller {
    ...
    //Update one item
    public function detail($p2h_id = NULL)
    {
        ...
        $data = array(
            'title' => 'P2H Detail',
            // 'kategori' => $this->m_kategori->get_all_data(),
            'p2h' => $this->m_p2h->get_data($p2h_id),
            'isi'   => 'p2h/v_p2h_detail'
        );
        $this->load->view('layout/v_wrapper_backend', $data, FALSE);
    ...
    public function operator1($id_histori = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            // if ($this->m_kendaraan->add_operator($data) == TRUE) {
                ...
                $this->m_kendaraan->edit_operator($data);
                $this->session->set_flashdata('pesan_kendaraan', 'Data Operator Ditambahkan');
                redirect('kendaraan');
            // }
        }
